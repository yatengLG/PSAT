...
class SettingDialog(QtWidgets.QDialog, Ui_Dialog):
    ...
    def get_item_and_widget(self, category, color: str):
        ...
        if category == '__unclassified__':
            color_button.setEnabled(True)
            delete_button.setEnabled(False)

        if category == '__ground__':
            color_button.setEnabled(True)
            delete_button.setEnabled(False)
    ...
    def load_cfg(self):
        ...
        if '__unclassified__' not in labels_key:
            labels.insert(0, {'name': '__unclassified__', 'color': '#ffffff'})

        if '__ground__' not in labels_key:
            labels.insert(1, {'name': '__ground__', 'color': '#fcaf3e'})
